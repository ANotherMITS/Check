# Reads MSTenantID from Site Variables 
  
if (($env:MSTenantID -as [string]).Length -gt 30) {
    $TenantId=$env:MSTenantID
    write-host "Using MS TentantID - " $env:MSTenantID
} else {
    write-host "No MS TenantID Defined"
    exit 
}
...
$enableGenericWebhook = 1 # 0 = Disabled, 1 = Enabled; default is 0; This will enable the generic webhook for sending detection events to a custom endpoint.
$webhookUrl = "https://logs.anotherit.co.uk/check?tenantId=$TenantId" # This will set the "Webhook URL" option; default is blank; if you set $enableGenericWebhook to 1, you must set this to a valid URL including the protocol (e.g., https://webhook.example.com/endpoint).
$webhookEvents = @("detection_alert", "false_positive_report", "page_blocked", "rogue_app_detected", "threat_detected", "validation_event") # This will set the "Event Types" to send to the webhook; default is blank; if you set $enableGenericWebhook to 1, you can specify which events to send. Available events: "detection_alert", "false_positive_report", "page_blocked", "rogue_app_detected", "threat_detected", "validation_event". Example: @("detection_alert", "page_blocked", "threat_detected").

# Custom Branding Settings
$companyName = "ANother Managed IT Services Ltd" # This will set the "Company Name" option in the Custom Branding settings; default is "CyberDrain".
$companyURL = "https://anotherit.co.uk" # This will set the Company URL option in the Custom Branding settings; default is "https://cyberdrain.com"; Must include the protocol (e.g., https://).
...
$supportEmail = "helpdesk@anotherit.co.uk" # This will set the "Support Email" option in the Custom Branding settings; default is blank.
$primaryColor = "#A23F97" # This will set the "Primary Color" option in the Custom Branding settings; default is "#F77F00"; must be a valid hex color code (e.g., #FFFFFF).
$logoUrl = "https://anotherit.co.uk/media/anotherit_logo_circle_128.png" # This will set the "Logo URL" option in the Custom Branding settings; default is blank. Must be a valid URL including the protocol (e.g., https://example.com/logo.png); protocol must be https; recommended size is 48x48 pixels with a maximum of 128x128.
...
function Configure-ExtensionSettings {
    ...
    $AllowListUrl = "https://raw.githubusercontent.com/ANotherMITS/Check/refs/heads/main/config/allowed_urls.json"

    try {
        Write-Host "Downloading URL allowlist..."

        $config = Invoke-RestMethod -Uri $AllowListUrl

        # Start with defaults
        $urlAllowlist = @($config.default)

        # Apply tenant-specific settings
        if ($config.tenants.PSObject.Properties.Name -contains $TenantId) {

            $tenantConfig = $config.tenants.$TenantId

            # Remove entries
            if ($tenantConfig.remove) {
                $urlAllowlist = $urlAllowlist | Where-Object {
                    $_ -notin $tenantConfig.remove
                }
            }

            # Add entries
            if ($tenantConfig.add) {
                $urlAllowlist += $tenantConfig.add
            }
        }
        ...
        Write-Host "Loaded $($urlAllowlist.Count) allowlist entries."

    }
    catch {
        Write-Warning "Unable to download allowlist from GitHub. Using built-in allowlist."
    }
